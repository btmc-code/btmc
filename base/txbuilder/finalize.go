...
func CalculateTxFee(tx *types.Tx) (fee uint64) {
	totalInputBTMC := uint64(0)
	totalOutputBTMC := uint64(0)

	for _, input := range tx.Inputs {
		if input.InputType() != types.CoinbaseInputType && input.AssetID() == *consensus.BTMCAssetID {
			totalInputBTMC += input.Amount()
		}
	}

	for _, output := range tx.Outputs {
		if *output.AssetId == *consensus.BTMCAssetID {
			totalOutputBTMC += output.Amount
		}
	}

	fee = totalInputBTMC - totalOutputBTMC
	return
}
