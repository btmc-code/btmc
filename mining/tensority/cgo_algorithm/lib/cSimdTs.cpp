...
BtminerMatList16* matList_int16;
uint8_t result[32] = {0};
map <vector<uint8_t>, BtminerMatList16*> seedCache;
...
uint8_t *SimdTs(uint8_t blockheader[32], uint8_t seed[32]){
    ...
    if(seedCache.find(seedVec) != seedCache.end()) {
        ...
        matList_int16 = new BtminerMatList16;
        matList_int16->init(extSeed);

        seedCache.insert(make_pair(seedVec, matList_int16));
    }

    iter_mineBtminer(blockheader, 32, result);

    if(seedCache.size() > cacheSize) {
        for(map<vector<uint8_t>, BtminerMatList16*>::iterator it=seedCache.begin(); it!=seedCache.end(); ++it){
            delete it->second;
        }
        seedCache.clear();
    }
