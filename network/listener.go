...
import (
	...
	"github.com/btmc-code/network/upnp"
)
...
func getUPNPExternalAddress(externalPort, internalPort int) (*NetAddress, error) {
	...
	externalPort, err = nat.AddPortMapping("tcp", externalPort, internalPort, "btmined tcp", 0)
	if err != nil {
		return nil, errors.Wrap(err, "could not add tcp UPNP port mapping")
	}
	externalPort, err = nat.AddPortMapping("udp", externalPort, internalPort, "btmined udp", 0)
	if err != nil {
...
//DefaultListener Implements btmined server Listener
